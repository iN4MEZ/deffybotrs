...
impl PatreonVerification {
    ...
    pub async fn verify(&self) -> Result<(bool,&String), Error> {
        ...
        Ok((verify.as_ref().is_some()
            && verify.as_ref().unwrap().patreon_status.is_some() && !(verify.as_ref().unwrap().patreon_status.unwrap() == PatronStatus::ActivePatron),
            &self.patreon_email)
        )
...
impl DiscordServerDatabaseManager {
    ...
    pub async fn set_webhook_channel(sv_id: u64,channel_id: u64,event: DatabaseWebHookEvent) -> Result<(),Error> {
        match event {
            DatabaseWebHookEvent::CreateMember => {
                DatabaseManager::update_discord_server_data(sv_id, "webhook_create_member_channel_id", channel_id).await?;
            } 
            DatabaseWebHookEvent::UpdateMember => {
                DatabaseManager::update_discord_server_data(sv_id, "webhook_update_member_channel_id", channel_id).await?;
            }
            DatabaseWebHookEvent::DeleteMember => {
                DatabaseManager::update_discord_server_data(sv_id, "webhook_delete_member_channel_id", channel_id).await?;
            }
        }
        Ok(())
