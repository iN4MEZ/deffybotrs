...
impl CommandHandler for EmbedCommand {
            ...
            match input.value.as_str() {
                Some("ctembed") => {
                    if let Some(content) = interaction.data.options.get(1) {
                        ...
                        interaction.channel_id.send_message(&ctx.http, rsp.0).await?;
                ...
                Some("editembed") => {
                    if let Some(content) = interaction.data.options.get(1) {
                        ...
                        let message_id_option = interaction.data.options.get(2)
                            .and_then(|opt| opt.value.as_str())
                            .and_then(|s| s.parse::<u64>().ok());

                        if let Some(message_id) = message_id_option {
                            use serenity::all::MessageId;
                            interaction.channel_id.edit_message(&ctx.http, MessageId::from(message_id), rsp.1).await?;
                        }
                    }
                }
    ...
    fn register(&self) -> CreateCommand {
        CreateCommand::new(self.name())
            ...
            .add_option(CreateCommandOption::new(serenity::all::CommandOptionType::String, "messageid", "A MessageId for edit embed"))
    }
}
...
fn generate_embed(content: CommandDataOption) -> Result<(CreateMessage,EditMessage),Error>  {
    if let Some(json_str) = content.value.as_str() {
        ...
        let create_message = CreateMessage::new()
            .add_embed(embed.clone()).components(components.clone());

        let edit_message = EditMessage::new()
        .add_embed(embed).components(components);
